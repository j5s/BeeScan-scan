...
func (p *Probe) getDirectiveSyntax(data string) (directive Directive) {
	...
	if strings.Count(data, " ") <= 0 {
		panic("nmap-probes - error directive format")
	}
...
func (p *Probe) parseProbeInfo(probeStr string) {
	...
	if len(other) == 0 {
		panic("nmap-probes - bad probe name")
	}
...
func (v *VScan) parseProbesFromContent(content string) {
	...
	// 判断第一行是否为 "Exclude " 设置
	if len(lines) == 0 {
		panic("Failed to read nmap-probes file for probe data, 0 lines read.")
	}
	c := 0
	for _, line := range lines {
		...
		if c > 1 {
			panic("Only 1 Exclude directive is allowed in the nmap-probes file")
		}
	}
	l := lines[0]
	if !(strings.HasPrefix(l, "Exclude ") || strings.HasPrefix(l, "Probe ")) {
		panic("Parse error on nmap-probes file: line was expected to begin with \"Probe \" or \"Exclude \"")
	}
	if c == 1 {
...
func (v *VScan) Init(f embed.FS) {
	var content string

	// 读取 nmap-probes 或自定义规则文件
	if data, err := f.ReadFile("nmap-probes"); err == nil {
		content = string(data)
	} else {
		panic(err)
	}
