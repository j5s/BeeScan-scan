...
func CDNCheck(r *httpx.Response, ip string, canme string) string {
	// check header
	for k, _ := range r.Headers {
		if util.StrInSlice(strings.ToLower(k), cdnHeaderKeys) {
			return fmt.Sprintf("CDN(Header:%s)", k)
		}
	}
	// check canme
	for k, v := range cdnCnameKeywords {
		if strings.Contains(canme, k) {
			return fmt.Sprintf("CDN(Cname:%s)", v)
		}
	}
	// check ip
	b, err := ranger.Contains(net.ParseIP(ip))
	...
	if b {
		...
		s = strings.Trim(s, "/")
		return fmt.Sprintf("CDN(CIDR:%s)", s)
	}
